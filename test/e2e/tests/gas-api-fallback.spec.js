...
describe('Gas API fallback', function () {
  ...
  it('error message is displayed but gas recommendation is not displayed', async function () {
    await withFixtures(
      {
        fixtures: new FixtureBuilder().withNonEIP1559Network().build(),
      ...
      async ({ driver }) => {
        ...
        const error = await driver.isElementPresent({
          test: 'Network is busy. Gas prices are high and estimates are less accurate.',
        });

        assert.equal(error, true, 'Error message is displayed');
      },
    );
  });
});
