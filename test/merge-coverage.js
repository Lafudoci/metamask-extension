...
async function start() {
  ...
  // from the jest-coverage/main directory.
  ...
  const coverageTargets = Object.keys(jestConfig.coverageThreshold);
      ...
      target === 'global'
        ...
        : getFileCoverage(
            glob.sync([
              ...
              '!**/*.test.js',
              // For some reason we aren't getting coverage for the files in
              // this folder, even though tests are run
              '!*app/scripts/controllers/permissions/flask/**/*.js',
            ]),
            coverageMap,
          );
    // Check and validate the coverage
    checkCoverage(target, jestConfig.coverageThreshold[target], summary);
  });
}
